...
static bool llama_pshard_probe_model_only(
    ...
    probe.n_layers    = model->hparams.n_layer_all;
...
static bool llama_pshard_params_supported(
    ...
    auto disable = [](const char * reason) {
        LLAMA_LOG_WARN("%s: %s, disabling pshard\n", "llama_params_fit_pshard_inference", reason);
        return false;
    };
...
void llama_params_fit_pshard_inference(
        const char * path_model,
    ...
    if (!pshard_registry_load_inference(registry, fp, cache_path.c_str(), host_buft, vram_free, false)) {
    ...
    llama_pshard_generate_overrides_inference(
