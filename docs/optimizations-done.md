...
| 2026-09-14 | 9875e7a11 | expert pool, scheduler, graph | hoist pass for every pshard tier (late = host-resident weights per layer + pool tensors); a CPU-computed boundary gets a split cut before the hoisted nodes and the scheduler's inputs-first CPU/GPU overlap runs on every tier, so the GPU shared experts run while the CPU expert chain works | q35 @8000 strategy 3 66.4 -> 67.7 t/s (+2.0%, 8 pairs), strategy 4 ~+0.5%, strategies 0/1/2 and pool unchanged; texts and gates identical; a plain reorder without the split cut measured 0 |
| 2026-09-14 | 0b01f0267 | runtime, scheduler, CUDA | transfer mode follows the active tier: kernel copies (cap = the profile's crossover) + async scheduler paths on any viable plan; prefetch copy backends keep the copy engine (per-backend deny list); sliced-expert uploads of a streamed MoE layer as one copy-segments launch | q35 @8000 s3 59.6 -> 66.3 t/s (+11%), s4 53.0 -> 58.8 (+11%), s1 36.4 -> 38.4 (+5.6%), s2 20.95 -> 21.1, s0 3.12 -> 3.145, pool fetch 98.6 unchanged; texts and gates identical |
...
- not landed transfer mode without the prefetch-backend deny list and the batched sliced upload: s3 +15%, s4 +1%, s1 -4%, s2 -4.5%; the two refinements landed instead (0b01f0267)
- not landed zero-copy join (device splits read CPU-split outputs through a device alias of the CPU compute buffer, no copy): engaged (28 result-copy kernels per token gone) but a wash - s3 65.9 -> 65.5, s2 21.15 -> 21.55, s4 and pool cells unchanged; after the kernel-copy transfer mode the copy it removes is a small kernel and the consumer's bus read replaces it; reverted
