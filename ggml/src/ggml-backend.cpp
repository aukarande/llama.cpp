...
void ggml_backend_tensor_memset_async(ggml_backend_t backend, struct ggml_tensor * tensor, uint8_t value, size_t offset, size_t size) {
    GGML_ASSERT(backend);
    GGML_ASSERT(tensor);
    GGML_ASSERT(tensor->data != NULL && "tensor not allocated");
    GGML_ASSERT(offset + size <= ggml_nbytes(tensor) && "tensor write out of bounds");
    ...
    if (backend->iface.memset_tensor_async == NULL) {
        ...
        backend->iface.memset_tensor_async(backend, tensor, value, offset, size);
    }
}
...
struct ggml_backend_sched {
    ...
    // fence for the asynchronous device->host input downloads of CPU splits (pshard layouts): recorded on
    // backend i's stream after each download, host-waited right before the CPU split computes. NULL on
...
static int ggml_backend_sched_backend_from_buffer(ggml_backend_sched_t sched, const struct ggml_tensor * tensor, const struct ggml_tensor * op) {
    ...
    // respect explicit set_tensor_backend assignments (walk view_src chain to root)
    {
        const struct ggml_tensor * cur = tensor;
        while (cur) {
            size_t idx = ggml_hash_find(&sched->hash_set, cur);
            if (idx != SIZE_MAX && idx < sched->hash_set.size) {
                int explicit_id = sched->hv_tensor_backend_ids[idx];
                if (explicit_id >= 0 && explicit_id < sched->n_backends) {
                    if (ggml_backend_supports_op(sched->backends[explicit_id], op)) {
                        return explicit_id;
                    }
                }
            }
...
static int ggml_backend_sched_backend_id_from_cur(ggml_backend_sched_t sched, struct ggml_tensor * tensor) {
    ...
    if (allow) {
        for (int i = 0; i < GGML_MAX_SRC; i++) {
            ...
            // pshard: device-resident shard buffers also anchor their consumers
            if (src->buffer != NULL && (src->buffer->usage == GGML_BACKEND_BUFFER_USAGE_WEIGHTS || !ggml_backend_buffer_is_host(src->buffer))) {
...
static bool ggml_backend_sched_split_produces_ids(
        const struct ggml_cgraph * g, const struct ggml_tensor * ids) {
...
static struct ggml_tensor * ggml_backend_sched_split_find_moe_consumer(
        const struct ggml_cgraph * g, const struct ggml_tensor * input_cpy) {
    for (int i = 0; i < g->n_nodes; i++) {
        struct ggml_tensor * node = g->nodes[i];
        if (node->op == GGML_OP_MUL_MAT_ID && node->src[0] == input_cpy &&
            node->src[2] != NULL && !ggml_backend_sched_split_produces_ids(g, node->src[2])) {
            return node;
        }
    }
    return NULL;
...
// main_graph != NULL: build-time call from split_graph, where forward candidates' split->graph
// views still hold the PREVIOUS invocation's graphs - construct current views from i_start/i_end
...
static int ggml_backend_sched_next_prefetch_split(struct ggml_backend_sched * sched, int i,
    ...
    for (int nid = i + 1; nid < lim; nid++) {
        ...
        struct ggml_cgraph cand_view;
        const struct ggml_cgraph * g;
        if (main_graph != NULL) {
            cand_view = ggml_graph_view(main_graph, candidate->i_start, candidate->i_end);
            g = &cand_view;
        } else {
            g = &candidate->graph;
        }
        if (ggml_backend_sched_split_has_prefetchable_weights(sched, candidate, g)) {
            return nid;
        }
    }
    return -1;
...
void ggml_backend_sched_split_graph(ggml_backend_sched_t sched, struct ggml_cgraph * graph) {
    ...
    for (int i = 0; i < graph->n_nodes; i++) {
        ...
        } else if (sched->redirect_target[*node_backend_id] < 0 && !sched->hv_tensor_usr[hash_id(node)]) {
            // assigned node: upgrade to higher prio backend if possible
            // skip for user-assigned nodes (set_tensor_backend)
            // skip for nodes on redirected backends
    ...
    // pass 5: split graph, find tensors that need to be copied
    {
        ...
        for (; i < graph->n_nodes; i++) {
            ...
            for (int j = 0; j < GGML_MAX_SRC; j++) {
                ...
                // collect registered writeback tensors for pre/post-compute callbacks
                // walk view_src to root, check hv_tensor_writeback hash
                {
                    ...
                    if (wb && split->n_writeback < GGML_SCHED_MAX_SPLIT_INPUTS) {
                        bool found = false;
    ...
    int total_writeback = 0;
    for (int i = 0; i < sched->n_splits; i++) {
        total_inputs    += sched->splits[i].n_inputs;
        total_writeback += sched->splits[i].n_writeback;
    }
    // pshard: prefetched weights add a third node per input; KV writeback
    // adds three nodes per writeback tensor
    const int nodes_per_input = sched->prefetch_weights ? 3 : 2;
    ...
    for (int i = 0; i < sched->n_splits; i++) {
        ...
        for (int w = 0; w < split->n_writeback; w++) {
            ...
            struct ggml_tensor * prealloc = ggml_view_tensor(sched->ctx, split->writeback[w]);
            prealloc->op = GGML_OP_NONE;
            prealloc->src[0] = split->writeback[w];
            sched->node_backend_ids[graph_copy->n_nodes] = split->backend_id;
        ...
        if (sched->prefetch_weights) {
            ...
            if (prefetch_outstanding < 0) {
                const int nid = ggml_backend_sched_next_prefetch_split(sched, i, graph);
                if (nid >= 0) {
            ...
            if (next_gpu != NULL) {
                ...
                // reserve next split's writeback slot (double-buffer: prefetch copy stream vs. current compute)
                for (int w = 0; w < next_gpu->n_writeback; w++) {
                    assert(graph_copy->size > graph_copy->n_nodes);
                    struct ggml_tensor * keepalive = ggml_view_tensor(sched->ctx, next_gpu->writeback[w]);
                    keepalive->op = GGML_OP_NONE;
                    keepalive->src[0] = next_gpu->writeback[w];
                    sched->node_backend_ids[graph_copy->n_nodes] = next_gpu->backend_id;
                    graph_copy->nodes[graph_copy->n_nodes++] = keepalive;
                }
            }
        ...
        // n_children fence for writeback (held through post_compute, required by FLAG_WRITEBACK semantics)
        for (int w = 0; w < split->n_writeback; w++) {
            assert(graph_copy->size > graph_copy->n_nodes);
            struct ggml_tensor * keepalive = ggml_view_tensor(sched->ctx, split->writeback[w]);
            keepalive->op = GGML_OP_NONE;
            keepalive->src[0] = split->writeback[w];
            sched->node_backend_ids[graph_copy->n_nodes] = split->backend_id;
            graph_copy->nodes[graph_copy->n_nodes++] = keepalive;
        }
    }
...
static enum ggml_status ggml_backend_sched_compute_splits(ggml_backend_sched_t sched) {
    ...
    auto produced_by_skipped = [&](const struct ggml_tensor * input) -> bool {
        for (int s : skipped_splits) {
            const struct ggml_cgraph * g = &sched->splits[s].graph;
            for (const struct ggml_tensor * t = input; t != NULL; t = t->view_src) {
    ...
    // backend_busy[i]: work may be queued on backends[i]'s stream since the sched last synchronized it.
    // Starts true (the application may have issued on any backend between graphs). The sched itself
    // only issues on a split's compute backend (a redirect target or the CPU) and on the copy backends,
    // never on an alias backend's own stream, so an alias stream drains at most once per graph.
    bool backend_busy[GGML_SCHED_MAX_BACKENDS];
    // pending_input_sync[i]: a CPU split's input is still downloading on backends[i]'s stream (the async
    // device->host path below); synchronized right before that CPU split computes
    bool pending_input_sync[GGML_SCHED_MAX_BACKENDS];
    // flag_input_pending[i]: a user input was uploaded asynchronously on backends[i] (pshard path below);
    // drained at the end of the graph, before the caller can overwrite the source
    bool flag_input_pending[GGML_SCHED_MAX_BACKENDS];
    for (int i = 0; i < sched->n_backends; i++) {
        backend_busy[i] = true;
        pending_input_sync[i] = false;
        flag_input_pending[i] = false;
    }
    ...
    auto host_upload_no_drain = [&](const struct ggml_tensor * input, const struct ggml_tensor * input_cpy,
        ...
        return pshard_async && sched->has_redirects && split_bid != sched->n_backends - 1 && input_cpy != input &&
               input->data != NULL && input_buf != NULL && ggml_backend_buffer_is_host(input_buf) &&
               ggml_backend_buffer_get_usage(input_buf) != GGML_BACKEND_BUFFER_USAGE_WEIGHTS &&
               sched->events[split_bid][sched->cur_copy] == NULL;
    };
    ...
    auto run_split = [&](int split_id, int stage) -> enum ggml_status {
        ...
        // everything this split issues (input copies, callbacks, compute, event records) lands on split_backend
        backend_busy[split_backend_id] = true;
        ...
        if (weights_prefetched && sched->copy_events[copy_backend_id] != NULL) {
            ggml_backend_event_wait(split_backend, sched->copy_events[copy_backend_id]);
        }
        if (weights_prefetched) {
            prefetched_split_id = -1;
            prefetched_backend_id = -1;
        ...
        auto prefetch_next_split = [&]() {
            ...
            int next_gpu_id = -1;
            {
                const int nid = ggml_backend_sched_next_prefetch_split(sched, split_id, NULL);
                if (nid >= 0) {
                    next_gpu = &splits[nid];
                    next_gpu_id = nid;
                }
            ...
            if (did_prefetch) {
                ggml_backend_event_record(sched->copy_events[next_gpu->backend_id], next_copy);
                prefetched_split_id = next_gpu_id;
                prefetched_backend_id = next_gpu->backend_id;
            }
        };
        ...
        for (int input_id = 0; input_id < split->n_inputs; input_id++) {
            ggml_backend_t input_backend = ggml_backend_sched_get_tensor_backend(sched, split->inputs[input_id]);
            {
                ...
                if (input_bid >= 0 && sched->redirect_target[input_bid] >= 0) {
                    input_backend = sched->backends[sched->redirect_target[input_bid]];
                }
            }
            ...
            if (input->flags & GGML_TENSOR_FLAG_INPUT) {
                ...
                if (split->graph.n_nodes > 0 && node != NULL && input_buf != NULL &&
                    ...
                    if (ids_tensor != prev_ids_tensor) {
                        ...
                        for (int64_t i1 = 0; i1 < ids_tensor->ne[1]; i1++) {
                            for (int64_t i0 = 0; i0 < ids_tensor->ne[0]; i0++) {
                                ...
                                if (id < 0) {
                                    continue; // -1 route (skipped expert): nothing to upload
                                }
                    ...
                    bool copied = split_backend->iface.cpy_tensor_async &&
                        split_backend->iface.cpy_tensor_async(input_backend, split_backend, input, input_cpy);
                    if (!copied && pshard_async && sched->has_redirects && split_backend_id == sched->n_backends - 1 &&
                        input->data != NULL && input->buffer != NULL && !ggml_backend_buffer_is_host(input->buffer) &&
        ...
        if (!sched->callback_eval) {
            if (sched->split_skip_cb != NULL && split->n_writeback == 0 &&
...
ggml_backend_sched_t ggml_backend_sched_new(
    ...
    // auto-detect same-device backends for compute redirect
    sched->has_redirects = false;
    for (int i = 0; i < n_backends; i++) {
        sched->redirect_target[i] = -1;
        ...
        for (int j = 0; j < i; j++) {
            if (ggml_backend_get_device(backends[j]) == dev_i) {
                sched->redirect_target[i] = j;
                sched->has_redirects = true;
                break;
            }
        }
        if (sched->redirect_target[i] < 0) {
            sched->d2h_events[i] = ggml_backend_event_new(dev_i);
        }
        // create copy backend + events for compute/transfer overlap
...
void ggml_backend_sched_set_prefetch_weights(ggml_backend_sched_t sched, bool enabled) {
    ...
    if (enabled) {
        for (int b = 0; b < sched->n_backends; b++) {
            if (sched->copy_backends[b] != NULL) continue;
...
void ggml_backend_sched_set_split_callbacks(
        ...
        void * user_data) {
    GGML_ASSERT(sched);
    sched->split_pre_compute  = pre_compute;
    sched->split_post_compute = post_compute;
    sched->split_cb_user_data = user_data;
}

void ggml_backend_sched_set_prefetch_cb(
        ggml_backend_sched_t sched,
...
void ggml_backend_sched_set_split_skip_cb(ggml_backend_sched_t sched,
...
void ggml_backend_sched_set_buffer(ggml_backend_sched_t sched, ggml_backend_t backend, ggml_backend_buffer_t buffer, size_t alloc_offset, size_t alloc_size) {
    GGML_ASSERT(sched);
    int backend_index = ggml_backend_sched_backend_id(sched, backend);
    GGML_ASSERT(backend_index >= 0 && backend_index < sched->n_backends);
    ggml_gallocr_set_buffer(sched->galloc, backend_index, buffer, alloc_offset, alloc_size);
}

void ggml_backend_sched_set_alloc_range(ggml_backend_sched_t sched, ggml_backend_t backend, size_t alloc_offset, size_t alloc_size) {
    GGML_ASSERT(sched);
    int backend_index = ggml_backend_sched_backend_id(sched, backend);
    GGML_ASSERT(backend_index >= 0 && backend_index < sched->n_backends);
    ggml_gallocr_set_alloc_range(sched->galloc, backend_index, alloc_offset, alloc_size);
}

int ggml_backend_sched_get_n_chunks(ggml_backend_sched_t sched, ggml_backend_t backend) {
    GGML_ASSERT(sched);
    int backend_index = ggml_backend_sched_backend_id(sched, backend);
    GGML_ASSERT(backend_index >= 0 && backend_index < sched->n_backends);
    return ggml_gallocr_get_n_chunks(sched->galloc, backend_index);
}

size_t ggml_backend_sched_get_chunk_max_size(ggml_backend_sched_t sched, ggml_backend_t backend, int chunk_id) {
    GGML_ASSERT(sched);
    int backend_index = ggml_backend_sched_backend_id(sched, backend);
    GGML_ASSERT(backend_index >= 0 && backend_index < sched->n_backends);
...
void ggml_backend_sched_save_backend_ids(ggml_backend_sched_t sched, int * node_buf, int * leaf_buf, int * n_nodes, int * n_leafs) {
    GGML_ASSERT(sched);
    int nn = sched->graph.n_nodes;
    int nl = sched->graph.n_leafs;
    if (n_nodes) *n_nodes = nn;
    if (n_leafs) *n_leafs = nl;
    if (node_buf) memcpy(node_buf, sched->node_backend_ids, nn * sizeof(int));
    if (leaf_buf) memcpy(leaf_buf, sched->leaf_backend_ids, nl * sizeof(int));
}
...
void ggml_backend_sched_restore_backend_ids(ggml_backend_sched_t sched, const int * node_buf, int n_nodes, const int * leaf_buf, int n_leafs) {
    ...
    if (leaf_buf && n_leafs > 0) {
        memcpy(sched->prev_leaf_backend_ids, leaf_buf, n_leafs * sizeof(int));
        memcpy(sched->leaf_backend_ids,      leaf_buf, n_leafs * sizeof(int));
    }
}

bool ggml_backend_sched_get_split_info(
        ggml_backend_sched_t sched, int split_id,
        struct ggml_backend_sched_split_info * out) {
    GGML_ASSERT(sched && out);
    if (split_id < 0 || split_id >= sched->n_splits) return false;

    struct ggml_backend_sched_split * s = &sched->splits[split_id];
    out->graph      = &s->graph;
...
void ggml_backend_sched_set_tensor_backend(ggml_backend_sched_t sched, struct ggml_tensor * node, ggml_backend_t backend) {
    ...
    sched->hv_tensor_usr[hash_id(node)] = true;
    SET_CAUSE(node, "usr");

    // not clearing is_reset here: split_graph() already sets is_reset=false,
