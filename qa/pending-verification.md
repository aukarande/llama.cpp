...
9. **pshard+MTP LANDED 2026-09-01** (overnight; commits 3c8e0a48c ddb019848
   ...
   limit) so DSv4 streaming runs on pageable copies - correct, slower. MEASURED 2026-09-02 (q35 forced s0, GGML_CUDA_REGISTER_HOST=0 now forces pageable): pageable uploads are 3.3x slower (prompt 638 -> 189 t/s, decode 2.33 -> 0.76 t/s), i.e. ~13.5 vs 45 GB/s. It does NOT affect DSv4 @12000 today: the plan is STATIC_ATTNPRIO at every tier (attention pinned, experts on CPU), nothing streams. It would matter for a streaming plan (half the bytes pageable -> ~21 GB/s effective) and the planner prices ALL uploads at 45 GB/s - an honesty gap for such plans (follow-up: per-mapping lockability -> per-layer upload rate in the predictor). ATTRIBUTED 2026-09-02 on the streaming prefill itself (DSv4 @12000 without a draft picks GPUONLY_ATTNPIN_FFNSTREAM for tiers 512-2048, predicted 118 t/s; 343-token prompt, PSHARD_PAGELOCK_SKIP lever): shard-2 pinned/shard-3 pageable 12.1 s (28.3 t/s); shard-3 pinned/shard-2 pageable 14.2 s; both pageable 26.5 s (super-additive: 92 GB unlocked no longer fits the page cache). Each shard's experts (42.3 / 41.3 GiB) take ~1.0 s pinned at 45 GB/s, so uploads are ~2 s of the 12 s and the remaining ~7-8 s is GPU-side MoE prefill cost (43 layers x 256 experts streamed; the predictor's 118 t/s assumes ~3 s). Projected all-pinned: ~9.4 s (36 t/s, +30%); a staging ring recovering ~70% of the pageable penalty: ~9.9 s (+22%); partial pinning of shard 3: proportional. The real DSv4 prefill lever is the ~7 s of non-upload time per 343-token batch (per-expert dispatch / IQ dequant on GPU?) and the predictor's 2.5x optimism there - a separate perf item. The loader now WARNs when a page-lock fails; a
